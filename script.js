...
import { loginWithGoogle, logout, onAuthChange, pushToCloud, pullFromCloud, mergeWithCloud,
  ...
  addFriendToMyList, getFriendStats,
...
function registerSW(){
  ...
  navigator.serviceWorker.register('/HVstudio1/service-worker.js').then(reg=>{
...
// ── Parche confiable: como el temporizador del Service Worker se puede "dormir",
// avisamos apenas la app está abierta (o sigue abierta) después de cierta hora,
// si todavía no llegaste al 100% del día. Como mucho una vez por día. ──
const REMINDER_HOUR=20; // hora local a partir de la cual se puede avisar
function checkInstantReminder(){
  if(!('Notification' in window)||Notification.permission!=='granted')return;
  if(isTodayRestDay())return; // día libre: no hay nada que recordar
  if(new Date().getHours()<REMINDER_HOUR)return;
  const key='dhv_reminder_shown_'+getTodayKey();
  if(localStorage.getItem(key)==='1')return;
  const pct=calcTodayPct();
  if(pct>=100)return;
  localStorage.setItem(key,'1');
  try{
    new Notification('DesafioHV ⭐ — Recordatorio',{
      body:`Vas ${pct}% del día. ¿Te faltan actividades por marcar?`,
      tag:'dhv-daily',
    });
  }catch(e){ /* algunos navegadores móviles no permiten new Notification() en foreground */ }
}
...
// Mínimo de actividades que debe tener un día para que cuente como "100% / día perfecto"
// para racha, logros y misiones. Evita que se pueda "farmear" con una sola actividad.
const MIN_ACTS_FOR_PERFECT=3;
// Un día "genuinamente perfecto" para logros/misiones: NO cuenta si es día libre
// (ahí no había nada que cumplir, así que no debería dar logros de 100%).
function isTodayGenuinelyPerfect(){
  return !isTodayRestDay() && activities.length>=MIN_ACTS_FOR_PERFECT && calcTodayPct()===100;
}
...
// ══════════════════════════════════════════════════════════
const MISSION_POOL=[
  {id:'all_done',icon:'🏆',name:'Día perfecto',desc:'Completá el 100% de tus actividades (mín. 3).',stars:2,check:()=>isTodayGenuinelyPerfect()},
  {id:'no_skip',icon:'🎯',name:'Sin excusas',desc:'100% del día sin ningún "No hice" (mín. 3 actividades).',stars:3,check:()=>isTodayGenuinelyPerfect()&&activities.every(a=>!isSkippedAct(a.id))},
  ...
  {id:'hard_perfect',icon:'☠️',name:'Sin compasión',desc:'Día al 100% en Modo Difícil.',stars:4,check:()=>getMode()==='hard'&&isTodayGenuinelyPerfect()},
  {id:'medium_perfect',icon:'⚡',name:'Precisión media',desc:'Día al 100% en Modo Media.',stars:2,check:()=>getMode()==='medium'&&isTodayGenuinelyPerfect()},
...
const ACHIEVEMENTS=[
  ...
  {id:'first_day',icon:'📅',name:'Primer día',desc:'Completá un día al 100% (mín. 3 actividades).',cat:'Inicio',stars:1,check:()=>isTodayGenuinelyPerfect()||calcTotalPerfect()>=1},
  ...
  {id:'no_skip_day',icon:'🎖️',name:'Día impoluto',desc:'Día al 100% sin ningún "No hice" (mín. 3 actividades).',cat:'Perfección',stars:2,check:()=>isTodayGenuinelyPerfect()&&activities.every(a=>!isSkippedAct(a.id))},
  ...
  {id:'mode_medium_first',icon:'🟡',name:'Modo Media',desc:'Completá un día en Modo Media.',cat:'Modos',stars:2,check:()=>getMode()==='medium'&&isTodayGenuinelyPerfect()},
  ...
  {id:'mode_hard_perfect',icon:'💀',name:'Sin piedad',desc:'Día al 100% en Modo Difícil.',cat:'Modos',stars:4,check:()=>getMode()==='hard'&&isTodayGenuinelyPerfect()},
...
function renderProfile(){
  ...
  // Tarjeta de identidad social: @usuario y código de amigo (solo acá, no en Amigos & Desafíos)
  const idSlot=document.getElementById('profile-id-slot');
  if(idSlot){
    if(myProfile && myProfile.username){
      idSlot.style.display='flex';
      idSlot.innerHTML=`
        <div class="profile-id-text">
          <div class="profile-id-label">Código de amigo</div>
          <div class="profile-id-code">${myProfile.friendCode}</div>
        </div>
        <button class="profile-id-copy" onclick="copyFriendCode()">📋 Copiar</button>`;
    } else {
      idSlot.style.display='none';
    }
  }
...
// ══════════════════════════════════════════════════════════
window.openConfirm=function(title,msg,cb,okLabel='Confirmar',variant='default'){
  confirmCallback=cb;
  document.getElementById('confirm-title').textContent=title;
  document.getElementById('confirm-msg').textContent=msg;
  document.getElementById('confirm-ok-btn').textContent=okLabel;
  const box=document.querySelector('.confirm-box');
  box.classList.remove('confirm-variant-done','confirm-variant-skip');
  if(variant==='done')box.classList.add('confirm-variant-done');
  else if(variant==='skip')box.classList.add('confirm-variant-skip');
...
const TUT_PAGES=[
  ...
  {type:'howto',icon:'➕',title:'Creá tu rutina',steps:[
    {i:'➕',t:'Tocá el botón +',d:'Desde la pantalla "Hoy" para agregar una actividad nueva.'},
    {i:'🎨',t:'Elegí nombre, horario y color',d:'Cada actividad tiene su horario de inicio, fin y un color para identificarla rápido.'},
    {i:'✏️',t:'Editá cuando quieras',d:'Podés modificar una actividad mientras esté pendiente. Una vez marcada como hecha, queda fija ese día.'},
  ],btn:'Siguiente →'},
  {type:'howto',icon:'✅',title:'Marcá tu día',steps:[
    {i:'✅',t:'✓ = la hice',d:'Te va a preguntar "¿Seguro?" antes de confirmar. Una vez que decís que sí, no se puede deshacer.'},
    {i:'✕',t:'✕ = no la hice',d:'Si no llegaste a hacerla, marcala como "No hice". También es una decisión final, pensalo bien.'},
    {i:'⭐',t:'Ganás estrellas',d:'Cada actividad completada suma una estrella para gastar en la Tienda.'},
    {i:'🏆',t:'Día perfecto',d:'Necesitás al menos 3 actividades completadas al 100% para que el día cuente en tu racha y logros.'},
  ],btn:'Siguiente →'},
  {type:'howto',icon:'👥',title:'Amigos y desafíos',steps:[
    {i:'🔑',t:'Compartí tu código',d:'Lo encontrás en tu Perfil. Pasáselo a un amigo para que te agregue.'},
    {i:'👀',t:'Vas a ver su racha, no su rutina',d:'Por privacidad, de tus amigos solo vas a ver su racha, estrellas y horas dedicadas por día — nunca el detalle de sus actividades.'},
    {i:'⚔️',t:'Desafíos 1 vs 1',d:'Compará racha, estrellas o días perfectos con un amigo y motivate a mejorar.'},
  ],btn:'Siguiente →'},
...
function renderTutPage(){
  ...
  if(p.type==='welcome'){
    ...
    box.innerHTML=`<div class="tut-dots">${dots}</div><div class="tut-howto-title">${p.icon} ${p.title}</div><div class="tut-steps">${p.steps.map(s=>`<div class="tut-step"><div class="tut-icon">${s.i}</div><div class="tut-step-text"><strong>${s.t}</strong><span>${s.d}</span></div></div>`).join('')}</div><button class="tut-btn" onclick="goTutPage(${tutPage+1})">${p.btn}</button><button class="tut-skip-btn" onclick="pickModeAndStart('easy')">Saltar tutorial</button>`;
  }else{
...
function drawClock(){
  ...
  [...activities].sort((a,b)=>timeToMin(a.start)-timeToMin(b.start)).forEach(act=>{
    ...
    const segColor=act.done?'#2bd9a8':isSkipped?'#e2748a':isExpired?'#eba756':act.color;
    const opacity=act.done?0.82:isSkipped?0.72:isExpired?0.6:0.88;
    const strokeColor=act.done?'rgba(43,217,168,0.55)':isSkipped?'rgba(226,116,138,0.45)':'rgba(28,18,64,0.5)';
  ...
  html+=`<circle cx="${cx}" cy="${cy}" r="${rInner}" fill="rgba(28,18,64,0.95)" stroke="rgba(127,119,221,0.12)" stroke-width="1.5"/>`;
...
function calcStreak(){
  ...
  const todayActs=todayActivities();
  if(isTodayRestDay()||(todayActs.length>=MIN_ACTS_FOR_PERFECT&&calcTodayPct()===100)||(hasTodayShield()&&calcTodayPct()>0))streak=1;
  ...
  for(let i=1;i<=365;i++){const d=new Date(today);d.setDate(d.getDate()-i);const key=getDateKey(d);const entry=hist.find(h=>h.key===key);
    ...
    if((p===100&&entry.total>=MIN_ACTS_FOR_PERFECT)||(hadShield&&p>0))streak++;else break;
  }
  return streak;
}
...
function calcTotalPerfect(){return getHistory().filter(h=>!h.skipped&&!h.restDay&&calcSnapPct(h)===100&&h.total>=MIN_ACTS_FOR_PERFECT).length;}
window.toggleSkipAct=function(id){
  const act=activities.find(a=>a.id===id);if(!act)return;
  if(act.done){showToast('Ya completada, no se puede modificar');return;}
  if(isSkippedAct(id))return; // ya marcada — no se puede deshacer
  openConfirm('¿No hiciste esta actividad?',`Vas a marcar "${act.name}" como no realizada. No vas a poder deshacer esto después.`,()=>{
    setSkippedAct(id,true);render();showToast('✕ No realizada');
  },'Sí, no la hice','skip');
};
...
window.toggleAct=function(id){
  const act=activities.find(a=>a.id===id);if(!act)return;
  if(act.done)return; // ya marcada — no se puede deshacer
  if(!canMark(act)){showToast('🔒 No podés marcar ahora');return;}
  openConfirm('¿Hiciste esta actividad?',`Vas a marcar "${act.name}" como completada. No vas a poder deshacer esto después.`,()=>{
    confirmActDone(id);
  },'Sí, la hice','done');
};
function confirmActDone(id){
  const act=activities.find(a=>a.id===id);if(!act||act.done)return;
  act.done=true;
  if(isSkippedAct(id))setSkippedAct(id,false);
  ...
  if(isTodayGenuinelyPerfect()){setTimeout(()=>{launchConfetti();showToast('🏆 ¡100% del día!');},700);}
}
...
window.deleteAct=function(id){
  const act=activities.find(a=>a.id===id);if(!act)return;
  if(act.done){showToast('🔒 No podés eliminar una actividad ya completada hoy');return;}
  if(isSkippedAct(id)){showToast('🔒 No podés eliminar una actividad ya marcada como no realizada');return;}
  openConfirm('Eliminar','¿Seguro? No se puede deshacer.',()=>{activities=activities.filter(a=>a.id!==id);saveData();render();showToast('🗑 Eliminada');},'Eliminar');
};
...
registerSW();
checkInstantReminder();
setInterval(checkInstantReminder, 5*60*1000); // reintenta cada 5 min mientras la app siga abierta
...
// ── Recompensa de estrellas por completar un desafío con un amigo ──
// Ambos cobran la base por terminarlo (haya ganador o empate); el ganador
// se lleva además un plus extra. Se paga una sola vez por desafío (persistido
// en localStorage) para que no se pueda reclamar dos veces.
const CHALLENGE_REWARD_TIERS = {3:4, 7:10, 14:20, 30:45};
function challengeBaseReward(durationDays){ return CHALLENGE_REWARD_TIERS[durationDays] || 0; }
function challengeWinnerBonus(durationDays){ return Math.round(challengeBaseReward(durationDays)/2); }
function challengeRewardClaimed(id){ return localStorage.getItem('dhv_chal_reward_'+id)==='1'; }
function claimChallengeReward(c){
  if(!currentUser||c.status!=='finished'||challengeRewardClaimed(c.id))return;
  localStorage.setItem('dhv_chal_reward_'+c.id,'1'); // marcar primero: nunca se paga dos veces
  const base=challengeBaseReward(c.durationDays);
  if(!base)return;
  const isWinner=c.winnerUid===currentUser.uid;
  const total=base+(isWinner?challengeWinnerBonus(c.durationDays):0);
  addStars(total);
  showToast(isWinner?`🏆 +${total}⭐ ¡ganaste el desafío!`:`⚔️ +${total}⭐ por completar el desafío`);
  render();
}
...
async function tickChallenges() {
  ...
  for (const c of challengesCache) {
    if (c.status === 'finished') { claimChallengeReward(c); continue; }
...
// ── Perfil de amigo: solo racha, % de hoy y horas/día — nunca sus actividades ──
window.openFriendProfile = async function(uid, username) {
  const modal = document.getElementById('friend-profile-modal');
  const body = document.getElementById('friend-profile-modal-body');
  document.getElementById('friend-profile-title').textContent = `👤 @${username}`;
  modal.style.display = 'flex';
  body.innerHTML = `<div class="fp-loading">Cargando…</div>`;
  try {
    const s = await getFriendStats(uid);
    if (!s) { body.innerHTML = `<div class="fv-empty">No se pudo cargar este perfil.</div>`; return; }
    const modeLabels = { easy: '🟣 Fácil', medium: '🟡 Media', hard: '🔴 Difícil' };
    const maxMin = (s.activityStats && s.activityStats[0]) ? s.activityStats[0].perWeekMin : 0;
    const barsHTML = (s.activityStats && s.activityStats.length)
      ? s.activityStats.map(a => {
          const pct = maxMin ? Math.max((a.perWeekMin / maxMin) * 100, 3) : 3;
          return `<div class="ta-bar-row"><div class="ta-bar-label"><span class="ta-dot" style="background:${a.color}"></span><span class="ta-name">${a.name}</span><span class="ta-total">${fmtTime(a.perWeekMin)}/sem</span></div><div class="ta-bar-track"><div class="ta-bar-fill" style="background:${a.color};width:${pct}%"></div></div></div>`;
        }).join('')
      : `<div class="fp-no-acts">Todavía no completó actividades.</div>`;
    body.innerHTML = `
      <div class="fp-note">🔒 Por privacidad, solo se muestran nombre y horas por semana de sus actividades — nunca a qué hora del día las hace.</div>
      <div class="fp-grid">
        <div class="fp-stat"><div class="fp-stat-icon">🔥</div><div class="fp-stat-val">${s.streak}</div><div class="fp-stat-lbl">Racha actual</div></div>
        <div class="fp-stat"><div class="fp-stat-icon">🏆</div><div class="fp-stat-val">${s.totalPerfectDays}</div><div class="fp-stat-lbl">Días perfectos</div></div>
        <div class="fp-stat"><div class="fp-stat-icon">⭐</div><div class="fp-stat-val">${s.totalStars}</div><div class="fp-stat-lbl">Estrellas</div></div>
        <div class="fp-stat"><div class="fp-stat-icon">⏱️</div><div class="fp-stat-val">${s.hoursPerDay}h</div><div class="fp-stat-lbl">Dedica por día</div></div>
      </div>
      <div class="fp-extra">
        <div class="fp-extra-row"><span>Modo actual</span><strong>${modeLabels[s.mode]||modeLabels.easy}</strong></div>
        <div class="fp-extra-row"><span>Progreso de hoy</span><strong>${s.todayPct===null?'Sin actividades':s.todayPct+'%'}</strong></div>
      </div>
      <div class="fp-acts-title">📊 Horas por semana por actividad</div>
      <div class="ta-bars fp-acts-bars">${barsHTML}</div>`;
  } catch (err) {
    body.innerHTML = `<div class="fv-empty">No se pudo cargar este perfil.</div>`;
  }
};
...
function renderChallengeModal() {
  ...
  const rewardBase = challengeBaseReward(cmSelectedDuration), rewardBonus = challengeWinnerBonus(cmSelectedDuration);
  ...
  body.innerHTML = `
    ...
    <div class="cm-reward-preview">🎁 Recompensa: <strong>${rewardBase}⭐</strong> para los dos al completarlo · <strong>${rewardBase+rewardBonus}⭐</strong> si ganás</div>
...
function renderFriendsView() {
  ...
  // ── Sub-tabs (tu @usuario y código ahora viven en Perfil) ──
  ...
  container.innerHTML = `
    <div class="fv-sticky-top">${subtabs}</div>
...
function renderChallengeCard(c) {
  ...
  const iAmLeading = myVal > theirVal, theyLead = theirVal > myVal;
  ...
  // ── Mensaje de ventaja, mientras el desafío está en curso ──
  let leadHTML = '';
  if (c.status === 'active') {
    const diff = Math.abs(myVal - theirVal);
    if (iAmLeading) leadHTML = `<div class="fc-lead mine">🔥 Vas arriba por ${diff}</div>`;
    else if (theyLead) leadHTML = `<div class="fc-lead theirs">😬 @${theirName} te lleva ${diff} de ventaja</div>`;
    else leadHTML = `<div class="fc-lead tie">🤝 Van iguales</div>`;
  }

  let resultHTML = '';
  const base = challengeBaseReward(c.durationDays), bonus = challengeWinnerBonus(c.durationDays);
  if (c.status === 'finished') {
    if (!c.winnerUid) resultHTML = `<div class="fc-result lose">🤝 Empate · +${base}⭐ para los dos</div>`;
    else if (c.winnerUid === currentUser.uid) resultHTML = `<div class="fc-result win">🏆 ¡Ganaste! +${base+bonus}⭐</div>`;
    else resultHTML = `<div class="fc-result lose">😔 Ganó @${theirName} · +${base}⭐ por participar</div>`;
  ...
  const iWonFinished = finished && c.winnerUid === currentUser.uid;
  const theyWonFinished = finished && c.winnerUid && c.winnerUid !== currentUser.uid;
  ...
  return `<div class="fc-card ${c.status}">
    ...
    <div class="fc-card-body">
      <div class="fc-duel">
        <div class="fc-side ${iAmLeading||iWonFinished?'leading':''}">
          <div class="fc-avatar mine">${(myProfile&&myProfile.username?myProfile.username[0]:'V').toUpperCase()}${(iAmLeading&&showBar)||iWonFinished?'<span class="fc-crown">👑</span>':''}</div>
          <div class="fc-side-name">Vos</div>
          <div class="fc-side-val ${iAmLeading||iWonFinished?'ahead':''}">${myVal}</div>
        </div>
        <div class="fc-vs-mid">VS</div>
        <div class="fc-side ${theyLead||theyWonFinished?'leading':''}">
          <div class="fc-avatar theirs">${(theirName||'U')[0].toUpperCase()}${(theyLead&&showBar)||theyWonFinished?'<span class="fc-crown">👑</span>':''}</div>
          <div class="fc-side-name">@${theirName}</div>
          <div class="fc-side-val ${theyLead||theyWonFinished?'ahead':''}">${theirVal}</div>
        </div>
      </div>
      ${showBar ? `<div class="fc-bar"><div class="fc-bar-mine ${iAmLeading?'lead':theyLead?'behind':'even'}" style="width:${myPct}%"></div><div class="fc-bar-theirs ${theyLead?'lead':iAmLeading?'behind':'even'}" style="width:${theirPct}%"></div><div class="fc-bar-mid"></div></div>` : ''}
      ${leadHTML}
      ${metaHTML}
      ${actionsHTML}
      ${resultHTML}
  ...
  if (!friendsListCache.length) {
    ...
    html += friendsListCache.map(f => `<div class="fv-friend-card">
      ...
      <div class="fv-friend-btns">
        <button class="fv-friend-view" onclick="openFriendProfile('${f.uid}','${f.username}')" title="Ver perfil">👤</button>
